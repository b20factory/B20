...
export default function Docs() {
    ...
    <main className="wrap py-12 grid lg:grid-cols-[200px_1fr] gap-10">
      ...
      <aside className="hidden lg:block sticky top-20 self-start text-sm">
        ...
        <ul className="space-y-1">
          {NAV.map(([id, label]) => (
            <li key={id}>
              <a href={id.startsWith("/") ? id : `#${id}`} className={`transition-colors ${id.startsWith("/") ? "text-beryl hover:text-beryl-glow" : "text-muted hover:text-text"}`}>{label}{id.startsWith("/") ? " ↗" : ""}</a>
            </li>
          ))}
        </ul>
      </aside>
      ...
      <article className="prose-term max-w-3xl">
        ...
        <ul>
          ...
          <li>Volatility-based dynamic swap fee, 1% to 5%</li>
        </ul>
        ...
        <ul>
          ...
          <li>Choose your fee band (base + max, each 1 to 5%)</li>
  ...
  --mc 10000`}</code></pre>
        ...
        <p>
          ...
          1% to 5%). In calm markets it sits at the base; when a swap moves price hard
